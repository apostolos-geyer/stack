...
- Package naming uses `@_/` prefix; path aliases defined per package (e.g., `@_/ui.web/*`). Prefer keeping shared logic in libraries (`lib.*`, `features.*`) and clean imports from infra (`@_/features/auth`, `@_/infra.db`).
