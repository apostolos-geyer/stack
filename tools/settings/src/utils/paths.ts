...
export const PATHS = {
  // infra.db package
  DB_PACKAGE_JSON: path.join(REPO_ROOT, 'packages/infra.db/package.json'),
  DB_CLIENT_TS: path.join(REPO_ROOT, 'packages/infra.db/src/client.ts'),
  DB_SCHEMA_PRISMA: path.join(REPO_ROOT, 'packages/infra.db/prisma/schema.prisma'),
  DB_PRISMA_CONFIG: path.join(REPO_ROOT, 'packages/infra.db/prisma.config.ts'),
  DB_MIGRATIONS: path.join(REPO_ROOT, 'packages/infra.db/prisma/migrations'),
  DB_README: path.join(REPO_ROOT, 'packages/infra.db/README.md'),
  DB_DOCKER_COMPOSE: path.join(REPO_ROOT, 'packages/infra.db/docker-compose.yml'),
